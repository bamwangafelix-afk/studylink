...
async function startVoice(fromGesture=false){
  ...
  try{
    ...
    recChatId=curChat&&CU?getCID(CU.uid,curChat.uid):null;
    if(recChatId)await setPresenceState('private',recChatId,'recording',true);
...
async function startGVoice(fromGesture=false){
  ...
  try{
    ...
    recGroupId=curGrp&&CU?curGrp.id:null;
    if(recGroupId)await setPresenceState('group',recGroupId,'recording',true);
    void keepVoiceScreenOn();
...
function setupPWA(){
  ...
  navigator.serviceWorker.register('sw-v11.js?v=studylink-pwa-11',{scope:'./',updateViaCache:'none'}).then(reg=>{
    reg.update().catch(()=>{});
