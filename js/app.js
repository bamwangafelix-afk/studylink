...
function onMsgInput(){
  ...
  db.collection('chats').doc(cid).set({ty:{[CU.uid]:true}},{merge:true});
  clearTimeout(typDebounce);
  typDebounce=setTimeout(()=>db.collection('chats').doc(cid).set({ty:{[CU.uid]:false}},{merge:true}),2000);
}
