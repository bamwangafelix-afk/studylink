...
test('uses large explicit non-submit microphone controls', async () => {
  ...
  assert.match(html, /<script defer src="js\/app-v20\.js\?v=studylink-pwa-58"><\/script>/);
  assert.match(html, /<link rel="stylesheet" href="css\/styles\.css\?v=studylink-pwa-58">/);
...
test('opens the selected profile from the Statut viewer and preserves modal history', () => {
  assert.match(source, /function viewStatusProfile\(\)[\s\S]*closeStatusView\(true\)[\s\S]*openProfile\(uid,profileUser\)/);
  assert.match(source, /function closeStatusView\(preserveHistory=false\)/);
  assert.match(source, /function openProfile\(uid,profileData=null\)[\s\S]*pushModalState\(\)/);
});

test('applies the requested visual refinements without regressing the pwa-58 Statut controls', async () => {
  const html = await readFile(new URL('../index.html', import.meta.url), 'utf8');
  const css = await readFile(new URL('../css/styles.css', import.meta.url), 'utf8');
  assert.match(html, /class="stPhotoVector st-photo-icon"/);
  assert.match(html, /class="st-photo-add-label" data-i18n="st_add_photo"/);
  assert.match(html, /<div class="stVMsg" id="stVMsg"><\/div>\s*<div class="stVBadge" id="stVBadge"><\/div>/);
  assert.match(html, /class="profile-back-btn" onclick="closeProfileView\(\)"/);
  assert.match(html, /class="profile-header-title">Profil<\/div>/);
  assert.match(css, /body\.dark \.st-photo-icon,[\s\S]*body\.dark \.st-photo-add-label\{color:#fff!important;\}/);
  assert.match(css, /body\.dark #profileView \.chdr,[\s\S]*body\.dark #profileView #pvName\{color:#fff!important;\}/);
  assert.match(css, /\.stVMain #stVMsg \+ #stVBadge\{margin-top:16px;margin-bottom:0;\}/);
  assert.match(css, /\.ring-outline-givehelp\{box-shadow:0 0 0 3px #000;\}/);
  assert.match(css, /\.stRing\.ring-givehelp\{width:48px;height:48px;padding:3px;background:#000!important;box-shadow:none!important;\}/);
  assert.match(css, /\.stRing\.ring-givehelp>img,\.stRing\.ring-givehelp>\.stThumb,\.stRing\.ring-givehelp>\.stAvatar,\.stRing\.ring-givehelp>\.stFallback\{border:2px solid var\(--bg\)!important;\}/);
  assert.match(css, /\.statusStrip\{[^}]*padding:8px 0 8px 0/);
  assert.match(css, /\.stRing\{width:48px;height:48px/);
  assert.match(css, /#profileView #pvAvatar\.ring-outline-givehelp\{box-shadow:0 0 0 4px #000;\}/);
  assert.match(source, /const profileStatus=activeStatusOf\(u\);[\s\S]*profileRingClass/);
  assert.match(css, /body\.dark #profileView \.chdr \.profile-back-btn,body\.dark #profileView \.chdr \.profile-header-title\{color:#fff!important;\}/);
});

test('merges the user branch language switch without regressing pwa-58 Statut controls', async () => {
  const html = await readFile(new URL('../index.html', import.meta.url), 'utf8');
  ...
  assert.match(source, /const workerUrl=new URL\('sw-v48\.js\?v=studylink-pwa-58'/);
  assert.match(source, /function smartStatusReply\(\)/);
  assert.match(source, /function startStatusVoice\(fromGesture=false\)/);
});
...
test('ships an installable PWA shell with the supplied StudyLink icon', async () => {
  ...
  const worker = await readFile(new URL('../sw-v48.js', import.meta.url), 'utf8');
  const css = await readFile(new URL('../css/styles.css', import.meta.url), 'utf8');
  assert.match(html, /rel="manifest" href="manifest\.webmanifest\?v=studylink-pwa-58"/);
  assert.match(html, /icons\/studylink-192\.png\?v=studylink-pwa-58/);
  ...
  assert.match(source, /const workerUrl=new URL\('sw-v48\.js\?v=studylink-pwa-58',location\.href\)\.href/);
  ...
  assert.match(worker, /studylink-shell-v58/);
  assert.match(manifest, /"start_url": "\.\/\?source=pwa-58"/);
  assert.match(manifest, /studylink-512\.png\?v=studylink-pwa-58/);
  assert.match(worker, /self\.addEventListener\('fetch'/);
  assert.doesNotMatch(html, /data:image\/png;base64,/);
  assert.match(css, /#auth\{[^}]*background-color:#0d2f4d;background-image:url\('\.\.\/icons\/studylink-login-full-logo\.png\?v=studylink-pwa-58'\)/);
