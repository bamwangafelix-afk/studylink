...
test('uses large explicit non-submit microphone controls', async () => {
  ...
  assert.match(html, /<script defer src="js\/app-v20\.js\?v=studylink-pwa-37"><\/script>/);
  assert.match(html, /<link rel="stylesheet" href="css\/styles\.css\?v=studylink-pwa-37">/);
...
test('merges the user branch language switch without regressing pwa-37 Statut controls', async () => {
  const html = await readFile(new URL('../index.html', import.meta.url), 'utf8');
  ...
  assert.match(source, /const workerUrl=new URL\('sw-v37.js\?v=studylink-pwa-37'/);
  assert.match(source, /function smartStatusReply\(\)/);
  assert.match(source, /function startStatusVoice\(fromGesture=false\)/);
});
...
test('ships an installable PWA shell with the supplied StudyLink icon', async () => {
  ...
  const worker = await readFile(new URL('../sw-v37.js', import.meta.url), 'utf8');
  const css = await readFile(new URL('../css/styles.css', import.meta.url), 'utf8');
  assert.match(html, /rel="manifest" href="manifest\.webmanifest\?v=studylink-pwa-37"/);
  assert.match(html, /icons\/studylink-192\.png\?v=studylink-pwa-37/);
  ...
  assert.match(source, /const workerUrl=new URL\('sw-v37.js\?v=studylink-pwa-37',location\.href\)\.href/);
  ...
  assert.match(manifest, /"start_url": "\.\/\?source=pwa-37"/);
  assert.match(manifest, /studylink-512\.png\?v=studylink-pwa-37/);
  assert.match(worker, /self\.addEventListener\('fetch'/);
  assert.doesNotMatch(html, /data:image\/png;base64,/);
  assert.match(css, /#auth\{[^}]*background-color:#0d2f4d;background-image:url\('\.\.\/icons\/studylink-login-full-logo\.png\?v=studylink-pwa-37'\)/);
...
test('uses the generated black StudyLink app icon at both PWA sizes', async () => {
  ...
  assert.match(manifest, /studylink-192\.png\?v=studylink-pwa-37/);
  assert.match(manifest, /studylink-512\.png\?v=studylink-pwa-37/);
});
